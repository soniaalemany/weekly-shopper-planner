...
function row(item={}){const r=document.createElement('div');r.className='shopping-row'+(item.checked?' checked':'');r.dataset.id=item.id||'';r.dataset.source=item.source||'manual';r.innerHTML=`<input type="checkbox" ${item.checked?'checked':''}><input class="s-name" value="${item.name||''}" placeholder="Artículo"><input class="s-qty" value="${[item.quantity,item.unit].filter(Boolean).join(' ')||''}" placeholder="Cantidad"><button type="button" class="remove small" aria-label="Eliminar artículo" title="Eliminar artículo">🗑</button>`;r.querySelector('input[type=checkbox]').onchange=e=>r.classList.toggle('checked',e.target.checked);r.querySelector('.remove').onclick=()=>{r.remove();scheduleSave()};return r}
